...
reqCallback serverUrl email event items = do
  ...
  pure unit
  where
    mailingDescription = "Mailing generated by Chrome Extension."
    -- << >> --
    displayAlert msg = liftEff window >>= (liftEff <<< alert msg)
    -- << >> --
    asyncGetEmailProps = do
      (LastMailingData props) <- ExceptT $ getLastMailing serverUrl email items.authtoken
      case props.mailing_data of
        Nothing -> do
          (MailingData ml) <- ExceptT $ postNewMailing serverUrl email mailingDescription items.authtoken
          liftEff $ pasteLink Nothing ml.unsubscription_link
          pure unit
        (Just (MailingData ml)) ->                    
          if ml.is_link_subscribed
            then do
              liftEff $ pasteLink Nothing ml.unsubscription_link
    ...
    asyncPostLink (Left ajaxErr@(AjaxError err)) =
      case err.description of
        ...
        UnexpectedHTTPStatus  { status: StatusCode 404 } -> do
          (MailingData ml) <- ExceptT $ postNewLink serverUrl email mailingDescription items.authtoken
          liftEff $ pasteLink Nothing ml.unsubscription_link
          liftEff $ textAreaListener serverUrl email event
